...
- **Routing:** frontend on `Host(domain)` (router `pn-<id>`, same as single-service), backend on `Host(domain) && PathPrefix(/api)` (router `pn-<id>-api`, **priority 100** so /api wins) — same domain, one cert, **no extra DNS, no CORS**. The `/api` prefix is **not stripped**, so the frontend should call `/api/...` and the backend should serve its routes under `/api`. Backend listen port comes from the **`BACKEND_PORT` env var** (falls back to the project `Port`); `PORT` is injected per-container. **No rollback** for monorepos (Result has no single `ImageTag`, like compose).
